...
def transcribe_audio_in_chunks(audio_path: Path, chunk_length: int = 600, overlap: int = 10,provider:str='fireworks',model="whisper-v3") -> dict:
    """
    ...
    if provider == 'groq':
          api_key = os.getenv('GROQ_API_KEY') # colab
          #api_key = os.getenv("GROQ_API_KEY")  # uncomment this if local, must have .env

          client = Groq(api_key=api_key, max_retries=0)
    elif provider == 'fireworks':
          api_key = os.getenv('FIREWORKS_API_KEY') # colab
          #api_key = os.getenv("FIREWORKS_API_KEY")  # uncomment this if local, must have .env
    ...
    try:
        ...
        processed_path = convert_audio_ffmpeg(audio_path)
        try:
